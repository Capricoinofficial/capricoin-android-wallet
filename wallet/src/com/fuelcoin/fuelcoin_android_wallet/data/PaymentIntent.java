...
package com.fuelcoin.fuelcoin_android_wallet.data;
...
import com.fuelcoin.fuelcoin_android_wallet.util.GenericUtils;
...
public final class PaymentIntent implements Parcelable
{
	...
	public static PaymentIntent fromFuelcoinUri(@Nonnull final PeercoinURI fuelcoinjUri)
	{
		final Address address = fuelcoinjUri.getAddress();
		final Output[] outputs = address != null ? buildSimplePayTo(fuelcoinjUri.getAmount(), address) : null;
		final String bluetoothMac = (String) fuelcoinjUri.getParameterByName(Bluetooth.MAC_URI_PARAM);
		final String paymentRequestHashStr = (String) fuelcoinjUri.getParameterByName("h");
		final byte[] paymentRequestHash = paymentRequestHashStr != null ? base64UrlDecode(paymentRequestHashStr) : null;

		return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, fuelcoinjUri.getLabel(), bluetoothMac != null ? "bt:"
				+ bluetoothMac : null, null, fuelcoinjUri.getPaymentRequestUrl(), paymentRequestHash);
	}
