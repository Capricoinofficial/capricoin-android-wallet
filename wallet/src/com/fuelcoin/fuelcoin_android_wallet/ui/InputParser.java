...
import com.fuelcoin.fuelcoin_android_wallet.util.Io;
	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("fuelcoin:"))
			{
				try
				{
					final PeercoinURI fuelcoinjUri = new PeercoinURI(null, input);
					...
					handlePaymentIntent(PaymentIntent.fromFuelcoinUri(fuelcoinjUri));
				}
				catch (final PeercoinURIParseException x)
				{
					log.info("got invalid fuelcoinj uri: '" + input + "'", x);

					error(R.string.input_parser_invalid_fuelcoinj_uri, input);
				}
			}
