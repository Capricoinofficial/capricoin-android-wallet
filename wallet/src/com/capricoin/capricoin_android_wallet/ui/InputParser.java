	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("capricoin:"))
			{
				try
				{
					final PeercoinURI capricoinjUri = new PeercoinURI(null, input);
					...
					handlePaymentIntent(PaymentIntent.fromFuelcoinUri(capricoinjUri));
				}
				catch (final PeercoinURIParseException x)
				{
					log.info("got invalid capricoinj uri: '" + input + "'", x);

					error(R.string.input_parser_invalid_capricoinj_uri, input);
				}
			}
