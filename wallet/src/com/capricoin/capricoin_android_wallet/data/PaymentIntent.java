...
package com.capricoin.capricoin_android_wallet.data;
...
public final class PaymentIntent implements Parcelable
{
	...
	public static PaymentIntent fromFuelcoinUri(@Nonnull final PeercoinURI capricoinjUri)
	{
		final Address address = capricoinjUri.getAddress();
		final Output[] outputs = address != null ? buildSimplePayTo(capricoinjUri.getAmount(), address) : null;
		final String bluetoothMac = (String) capricoinjUri.getParameterByName(Bluetooth.MAC_URI_PARAM);
		final String paymentRequestHashStr = (String) capricoinjUri.getParameterByName("h");
		final byte[] paymentRequestHash = paymentRequestHashStr != null ? base64UrlDecode(paymentRequestHashStr) : null;

		return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, capricoinjUri.getLabel(), bluetoothMac != null ? "bt:"
				+ bluetoothMac : null, null, capricoinjUri.getPaymentRequestUrl(), paymentRequestHash);
	}
